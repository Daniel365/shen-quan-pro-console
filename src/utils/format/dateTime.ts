 ...
 * @Description: 时间格式化相关
 */
import { format, formatISO, formatRelative, isValid } from 'date-fns';
 ...
 */
export const formatIso86DateTime = (date: Date | number | string): string => {
  if (!date) {
 ...
 * @param date - 需要格式化的时间，可以是Date对象、时间戳或字符串
 * @returns ISO8601格式的日期字符串（带时区偏移），无效日期返回空字符串
 */
export const toISO8601WithCustomTimezone = (date: Date | number | string): string => {
  if (!date) {
    console.warn('无效的日期:', date);
    return '';
  }
