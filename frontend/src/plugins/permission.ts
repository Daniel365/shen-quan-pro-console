...
export function setupPermission() {
  // 白名单路由
  const whiteList: any[] = [RouterPath.LOGIN, RouterPath.REGISTER, RouterPath.FORGOT_PASSWORD];

  router.beforeEach(async (to, from, next) => {
    NProgress.start();

    const isLoggedIn = Storage.get(StorageKeyEnum.ACCESS_TOKEN);
...
export function hasAuth(value: string | string[], type: 'button' | 'role' = 'button') {
  ...
  // 超级管理员 拥有所有权限
  if (type === 'button' && isRoot(roleCode)) {
    return true;
  }
